...
public:
    ...
    {
		SIMDVector3 v;
		v.mVec = _mm_dp_ps(mVec, mVec, 0x77);
		float total = v.x;
		return total;
    }
    ...
    {
		float total = Length();
        return SIMDVector3(total, total, total);
    }

    // Length of vector
    float Length() const
    {
		float total = LengthSq();
    ...
    {
		SIMDVector3 v(_mm_dp_ps(a.mVec, b.mVec, 0x77));
        return v;
    }
    ...
    {
		//(ay, az, ax) * (bz, bx, by) - (az, ax, ay) * (by, bz, bx)
        return SIMDVector3(_mm_sub_ps(_mm_mul_ps(_mm_shuffle_ps(a.mVec, a.mVec, _MM_SHUFFLE(3, 0, 2, 1)), _mm_shuffle_ps(b.mVec, b.mVec, _MM_SHUFFLE(3, 1, 0, 2))),
			_mm_mul_ps(_mm_shuffle_ps(a.mVec, a.mVec, _MM_SHUFFLE(3, 1, 0, 2)), _mm_shuffle_ps(b.mVec, b.mVec, _MM_SHUFFLE(3, 0, 2, 1)))));
    }

    // Lerp from A to B by f
    friend SIMDVector3 Lerp(const SIMDVector3& a, const SIMDVector3& b, float f)
    {
		return a + SIMDVector3(_mm_sub_ps(b.mVec, a.mVec)) * f;
    }
...
public:
    ...
    float LengthSq() const
    {
        // TODO Lab 01h
		SIMDVector4 v;
		v.mVec = _mm_dp_ps(mVec, mVec, 0xFF);
		float total = v.x;
		return total;
    }
    ...
    SIMDVector4 Length_v() const
    {
        // TODO Lab 01h
		float total = Length();
		return SIMDVector4(total, total, total, total);
    }
    ...
    {
        // TODO Lab 01h
		SIMDVector4 v(_mm_dp_ps(a.mVec, b.mVec, 0xFF));
		return v;
    }
    ...
    {
        // TODO Lab 01h
		return SIMDVector4(_mm_sub_ps(_mm_mul_ps(_mm_shuffle_ps(a.mVec, a.mVec, _MM_SHUFFLE(3, 0, 2, 1)), _mm_shuffle_ps(b.mVec, b.mVec, _MM_SHUFFLE(3, 1, 0, 2))),
			_mm_mul_ps(_mm_shuffle_ps(a.mVec, a.mVec, _MM_SHUFFLE(3, 1, 0, 2)), _mm_shuffle_ps(b.mVec, b.mVec, _MM_SHUFFLE(3, 0, 2, 1)))));
    }
    ...
    {
        // TODO Lab 01h
		return a + SIMDVector4(_mm_sub_ps(b.mVec, a.mVec)) * f;
        return SIMDVector4::Zero;
    }
