...
Player::Player(SkinnedObj *pObj, AssetManager* am)
    : Character(am, pObj)
	, mMoveSpeed(0.0f)
...
{
	mObj = pObj;
	Matrix4 mat = mObj->mMatrix.perObjConst; // CHANGED
...
void Player::Update(float deltaTime)
{
    ...
    mPrevAnimTime = mCurrentTime; // CHANGED
...
bool Player::CheckGround(float* groundHeight)
{
    *groundHeight = 0.0f;
	Vector3 hitPoint;
	Physics::LineSegment segment(mPos + Vector3(0,0,10), mPos + Vector3(0,0,-10));
	bool intersect = mObj->GetGame()->GetPhysics()->RayCast(segment, &hitPoint);
...
void Player::ChangeState()
{
    ...
    case STATE_UNKNOWN:
        ...
        break;
    case STATE_LAND:
        if (mCurrentTime < mPrevAnimTime) // CHANGED
            SetState(STATE_IDLE);
    case STATE_IDLE:
...
void Player::UpdateState(float deltaTime)
{
    ...
    mPos += mVel * deltaTime;
    mObj->mMatrix.perObjConst = mat
        * Matrix4::CreateTranslation(mPos);  // CHANGED
}
